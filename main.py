...
print "Loading..."
mb.load_file("multiscale.h5m")

gid_tag = mb.tag_get_handle("GLOBAL_ID")
perm_tag = mb.tag_get_handle("perm")
#coarse_perm_tag = mb.tag_get_handle("coarse_perm")
injection_tag = mb.tag_get_handle("injection_well")
production_tag = mb.tag_get_handle("production_well")
...
perm_values = mb.tag_get_data(perm_tag, volumes)

v_ids = mb.tag_get_data(gid_tag, volumes).flatten()
v_ids = np.subtract(v_ids, np.min(v_ids))

std_map = Epetra.Map(len(volumes), 0, comm)
A = Epetra.CrsMatrix(Epetra.Copy, std_map, 0) #What if Epetra.VbrMatrix?????????

# Initial volume pressure data
pres_tag = mb.tag_get_handle("Pressure", 1, types.MB_TYPE_DOUBLE, True)
...
for idx, elem in zip(v_ids, volumes):
    ...
    if not boundary:
        ...
        K1 = mb.tag_get_data(perm_tag, [elem], flat=True)[0]

        #adj_perms = mb.tag_get_data(perm_tag, adj_volumes, flat=True)
        adj_perms = []
        for adjacencies in range(len(adj_volumes)):
            adj_perms.append(mb.tag_get_data(perm_tag,
...
if USE_DIRECT_SOLVER:
    ...
    outfile_template = "Results/output_iterative_10neg{0}.vtk"
